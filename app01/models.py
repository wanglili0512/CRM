...
class UserInfo(AbstractUser):
    ...
    depart = models.ForeignKey(verbose_name="所属部门", to="Department", default=1, on_delete=models.CASCADE)
    user_rbac = models.OneToOneField(verbose_name="rbac用户", to=User, null=True, on_delete=models.CASCADE)
...
class Customer(models.Model):
    ...
    course = MultiSelectField("咨询课程", choices=course_choices, blank=True, null=True)
    ...
    deal_date = models.DateField(null=True)   # 成单时间
    ...
    def get_status(self):
        ...
        return mark_safe("<span class='status_span' style='background-color:%s;color:white'>%s</span>" % (
...
class PaymentRecord(models.Model):
    ...
    status = models.IntegerField(verbose_name='审核', choices=status_choices, default=1)
...
class StudentStudyRecord(models.Model):
    ...
    class Meta:   # 学员和节次联合唯一，限制了每个班级学习记录只能初始化一遍学生学习记录
